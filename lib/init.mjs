...
export async function run(args) {
  ...
  // Create journal and corpus directories (empty, ready for use)
  mkdirSync(join(targetDir, "journal"), { recursive: true });
  mkdirSync(join(targetDir, "corpus"), { recursive: true });

  // Copy and process all templates
  copyTemplates(TEMPLATES_DIR, targetDir, vars);

  // Write config file (no secrets — those go in .env)
  const config = {
    name,
    api: {},
    process: {
  ...
  // Write .env.example as a guide for users
  const envExample = [
  ...
  console.log(`✓ 环境变量模板已写入 .env.example`);
  console.log(`
下一步:
  1. 复制 .env.example 为 .env 并填入实际的 API 地址、模型和密钥
  2. 在 journal/YYYY-MM-DD/ 下创建笔记，或在 corpus/<系列名>/ 下放入外部文章
