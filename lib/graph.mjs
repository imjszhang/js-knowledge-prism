...
const NODE_TYPES = ["journal", "corpus", "atom", "group", "synthesis", "perspective", "output"];
...
export function extractGraph(baseDir) {
  ...
  for (const series of listSeriesDirs(paths.corpusDir)) {
    ...
    for (const f of listCorpusFiles(seriesDir)) {
      const filePath = join(seriesDir, f);
      const relPath = relative(baseDir, filePath).replace(/\\/g, "/");
      const title = extractTitle(read(filePath));
      addNode(relPath, {
        name: `[${series}] ${title}`,
  ...
  // -- Layer 2: Atoms + journal/corpus→atom links --
  ...
  if (existsSync(paths.atomsDir)) {
    for (const sub of readdirSync(paths.atomsDir)) {
      ...
      if (!statSync(subDir).isDirectory() || !/^(\d{4}-\d{2}|corpus-.+)$/.test(sub)) continue;
      for (const f of listMdFiles(subDir)) {
        ...
        // Register atom IDs (XX-01 or XX-0010-01) for later group linking
        for (const m of content.matchAll(/\|\s*([A-Z]{2}-(?:\d{4}-)?\d{2})\s*\|/g)) {
          atomIdToPath.set(m[1], relPath);
        }

        // journal/corpus → atom link via "来源" line
        const sourceMatch = content.match(/>\s*来源[：:]\s*\[([^\]]*)\]\(([^)]+)\)/);
        if (sourceMatch) {
          const srcAbsPath = resolve(dirname(filePath), sourceMatch[2]);
          const srcRelPath = relative(baseDir, srcAbsPath).replace(/\\/g, "/");
          if (nodeIndex.has(srcRelPath)) {
            addLink(srcRelPath, relPath, LINK_TYPES.EXTRACT);
          }
        }
      }
    }
...
export function generateGraphHtml(graph, stats, options = {}) {
  ...
  if (writeIfChanged(outputPath, html)) {
    log(`✓ 已生成知识图谱: ${outputPath}`);
  } else {
    log(`  知识图谱无变更，跳过: ${outputPath}`);
  }
  return outputPath;
}
...
export function filterByPerspective(graph, perspectiveId) {
