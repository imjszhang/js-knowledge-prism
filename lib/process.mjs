...
// collectGroupedPrefixes is now imported from utils.mjs
 ...
 * @param {boolean}  [opts.forceSynthesis=false] - Force stage 3 even when stages 1/2 had no changes
...
export async function runPipeline({
  ...
  const hadChanges = summary.atomsProcessed > 0 || summary.groupsWritten > 0 || summary.groupsUpdated > 0;

  if (maxStage >= 3 && allGroupPaths.length > 0 && !hadChanges && !forceSynthesis && !dryRun) {
    log("\n本轮阶段 1/2 无实际变更，跳过阶段 3 synthesis 更新（可用 --force-synthesis 强制执行）");
...
选项:
  ...
  --force-synthesis    即使阶段 1/2 无变更也强制执行阶段 3
...
export async function run(args) {
  const { values: flags } = parseArgs({
    ...
    options: {
      ...
      "force-synthesis": { type: "boolean", default: false },
  ...
  return runPipeline({
    ...
    forceSynthesis: flags["force-synthesis"],
  });
}
