...
export function listRewrites(baseDir) {
  ...
  if (existsSync(BUILTIN_REWRITES_DIR)) {
    for (const f of readdirSync(BUILTIN_REWRITES_DIR).filter(isMd)) {
      const name = f.replace(/\.md$/, "");
      if (!seen.has(name)) {
...
示例:
  ...
  js-knowledge-prism rewrite --style <name> --file outputs/yangxia-series/P25/01.md
  js-knowledge-prism rewrite --style <name> --dir outputs/yangxia-series/P25/
  js-knowledge-prism rewrite --style <name> --dir outputs/yangxia-series/P25/ --review
`.trim();
